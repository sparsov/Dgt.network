...
DethState = namedtuple('DethState', ['name', 'url', 'value'])
...
class DethWorkload(Workload):
    ...
    def __init__(self, delegate, args):
        super(DethWorkload, self).__init__(delegate, args)
        ...
        if args.key_file is not None:
            try:
                ...
                raise DethCliException(str(pe))
            except IOError as ioe:
                raise DethCliException(str(ioe))
        else:
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        DethState(name=name, url=url, value=0)
...
def do_workload(args):
    """
    Create WorkloadGenerator and DethWorkload. Set Deth workload in
    generator and run.
    """
    try:
        args.auth_info = _get_auth_info(args.auth_user, args.auth_password)
        generator = WorkloadGenerator(args)
        workload = DethWorkload(generator, args)
