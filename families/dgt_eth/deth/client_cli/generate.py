...
def loads_deth_token(data,name=None):
...
def create_deth_transaction(verb, name, value, signer,to = None):
    payload = DethPayload(verb=verb, name=name, value=value, to = to)

    # The prefix should eventually be looked up from the
    # validator's namespace registry.
    addr = make_deth_address(name)
    inputs  = [addr]
    outputs = [addr]
    if to is not None:
        addr_to = make_deth_address(to)
