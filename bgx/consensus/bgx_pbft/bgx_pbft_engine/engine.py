...
from bgx_pbft_common.utils import _short_id,_SID_
...
class PbftEngine(Engine):
    ...
    def update_topology_settings(self):
        ...
        if self._cluster_name is None:
            ...
            LOGGER.debug("Genesis=%s(%s) Node=%s arbiters=%s",self._genesis,_SID_(self._genesis_node),self.arbiters_info)
        else:
            LOGGER.debug("Genesis=%s(%s) Node=%s %s in cluster=%s nodes=%s arbiters=%s(%s)",self._genesis,_SID_(self._genesis_node),_SID_(self._validator_id),self.own_type,self._cluster_name,[_SID_(key) for key in self._cluster.keys()],
    ...
    def check_consensus(self,blocks,block_num,summary,num_peers):
        ...
        if len(blocks) == (num_peers + 1) or not self._send_batches:                                                                                                                                
            ...
            LOGGER.debug("We have all prepares for block=%s SUMMARY=%s select=%s blocks=%s",block_num,summary[:8],selected[:8],[_SID_(key) for key in blocks.keys()])        
    ...
    def _handle_new_block(self, block):
        ...
        block_num = block.block_num
        num_peers = self.num_peers 
        LOGGER.info('=> NEW_BLOCK:Received block=%s.%s signer=%s summary=%s num_peers=%s SYNC=%s',block_num,_SID_(block_id),_SID_(signer_id),_SID_(summary),num_peers,self.is_sync)
        def check_consensus():
        ...
        if signer_id == self._validator_id:
            ...
            if block.previous_block_id in self._branches:
                ...
                if result == Consensus.done:
                    ...
                    LOGGER.info('   NEW_HEAD=%s for BRANCh=%s', _SID_(block_id),block.previous_block_id[:8])
                elif result == Consensus.fail:
                    LOGGER.info('Failed consensus check: %s', _SID_(block_id))
                else:
                    # consensus was started
                    LOGGER.info('consensus started for block=%s->%s', _SID_(block_id),block.previous_block_id[:8])
                    self._new_heads[block_id] = block.previous_block_id
    ...
    def _handle_peer_message(self, msg):
        ...
        LOGGER.debug("=> PEER_MESSAGE %s.'%s' block_id=%s(%s) summary=%s peer=%s.%s(%s)",info.seq_num,CONSENSUS_MSG[msg_type],_SID_(block_id),_SID_(signer_id),summary[:8],peer_nm,_SID_(peer_id),peer_status)
            ...
            if block_id in self._peers_branches:
                ...
                if block_num == 0:
                    ...
                    if summary in self._prepare_msgs:
                        ...
                        if block_id in blocks:
                            LOGGER.debug("=>IGNORE BLOCK=%s FOR SUMMARY=%s (already has).",_SID_(block_id),summary[:8])
                        else:
                            # add block into list and check number of blocks
                            num_peers = self.num_peers
                            blocks[block_id] = True
                            LOGGER.debug("=>ADD BLOCK=%s INTO SUMMARY=%s total=%s peers=%s",_SID_(block_id),summary[:8],len(blocks),num_peers) 
                            self.check_consensus(blocks,block_num,summary,num_peers)
                            
                    else:
                        LOGGER.debug("=>ADD BLOCK=%s INTO SUMMARY=%s",_SID_(block_id),summary[:8])
                ...
                if summary in self._prepare_msgs:
                    ...
                    if block_id not in blocks:
                         LOGGER.debug("=>ADD BLOCK=%s INTO SUMMARY=%s total=%s (no block yet)",_SID_(block_id),summary[:8],len(blocks))
                         blocks[block_id] = True
                    ...
                    if block_id in self._pre_prepare_msgs:
                        ...
                        LOGGER.debug("=>ADD BLOCK=%s INTO SUMMARY=%s (no block yet)",_SID_(block_id),summary[:8])
                        self._prepare_msgs[summary] = {block_id : True}
