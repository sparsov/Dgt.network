...
from sawtooth_bgt_simulator.journal.block_wrapper import NULL_BLOCK_IDENTIFIER
from sawtooth_bgt_simulator.bgt_enclave_simulator.utils import json2dict
from sawtooth_bgt_simulator.bgt_enclave_simulator.utils import dict2json

from sawtooth_bgt_simulator.bgt_enclave_simulator.enclave_signup_info \
    import EnclaveSignupInfo
from sawtooth_bgt_simulator.bgt_enclave_simulator.enclave_wait_timer \
    import EnclaveWaitTimer
from sawtooth_bgt_simulator.bgt_enclave_simulator.enclave_wait_certificate \
...
class _BgtEnclaveSimulator:
    ...
    def initialize(cls, config_dir, data_dir):
        ...
        config_file = os.path.join(config_dir, 'bgt_enclave_simulator.toml')
        if os.path.exists(config_file):
            LOGGER.info(
                'Loading BGT enclave simulator config from : %s',
                config_file)
            ...
            try:
                ...
                LOGGER.info(
                    'Error loading BGT enclave simulator configuration: %s',
                    e)
            ...
            if invalid_keys:
                LOGGER.warning(
                    'Ignoring invalid keys in BGT enclave simulator config: '
                    '%s',
        ...
        LOGGER.debug(
            'BGT enclave simulator creating anti-Sybil ID from: %s',
            validator_id)
...
def initialize(config_dir, data_dir):
    _BgtEnclaveSimulator.initialize(config_dir=config_dir, data_dir=data_dir)


def shutdown():
    _BgtEnclaveSimulator.shutdown()


def get_enclave_measurement():
    return _BgtEnclaveSimulator.get_enclave_measurement()


def get_enclave_basename():
    return _BgtEnclaveSimulator.get_enclave_basename()


def create_signup_info(originator_public_key_hash,
                       nonce):
    return \
        _BgtEnclaveSimulator.create_signup_info(
            originator_public_key_hash=originator_public_key_hash,
            nonce=nonce)


def deserialize_signup_info(serialized_signup_info):
    return _BgtEnclaveSimulator.deserialize_signup_info(
        serialized_signup_info=serialized_signup_info)


def unseal_signup_data(sealed_signup_data):
    return _BgtEnclaveSimulator.unseal_signup_data(sealed_signup_data)


def release_signup_data(sealed_signup_data):
    return _BgtEnclaveSimulator.release_signup_data(sealed_signup_data)
...
def create_wait_timer(sealed_signup_data,
                      ...
                      local_mean):
    return \
        _BgtEnclaveSimulator.create_wait_timer(
            sealed_signup_data=sealed_signup_data,
            validator_address=validator_address,
            previous_certificate_id=previous_certificate_id,
            local_mean=local_mean)
...
def deserialize_wait_timer(serialized_timer, signature):
    return \
        _BgtEnclaveSimulator.deserialize_wait_timer(
            serialized_timer=serialized_timer,
            signature=signature)
...
def create_wait_certificate(sealed_signup_data, wait_timer, block_hash):
    return \
        _BgtEnclaveSimulator.create_wait_certificate(
            sealed_signup_data=sealed_signup_data,
            wait_timer=wait_timer,
            block_hash=block_hash)
...
def deserialize_wait_certificate(serialized_certificate, signature):
    return \
        _BgtEnclaveSimulator.deserialize_wait_certificate(
            serialized_certificate=serialized_certificate,
            signature=signature)


def verify_wait_certificate(certificate, bgt_public_key):
    _BgtEnclaveSimulator.verify_wait_certificate(
        certificate=certificate,
        bgt_public_key=bgt_public_key)
