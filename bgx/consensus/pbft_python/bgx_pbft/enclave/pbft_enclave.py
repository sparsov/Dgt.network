...
from bgx_pbft.enclave.enclave_signup_info import EnclaveSignupInfo

#from sawtooth_bgt_simulator.bgt_enclave_simulator.enclave_wait_timer import EnclaveWaitTimer
#from sawtooth_bgt_simulator.bgt_enclave_simulator.enclave_wait_certificate import EnclaveWaitCertificate
...
def initialize(config_dir, data_dir):
    _PbftEnclaveSimulator.initialize(config_dir=config_dir, data_dir=data_dir)


def shutdown():
    _PbftEnclaveSimulator.shutdown()


def get_enclave_measurement():
    return _PbftEnclaveSimulator.get_enclave_measurement()


def get_enclave_basename():
    return _PbftEnclaveSimulator.get_enclave_basename()


def create_signup_info(originator_public_key_hash,
                       nonce):
    return \
        _PbftEnclaveSimulator.create_signup_info(
            originator_public_key_hash=originator_public_key_hash,
            nonce=nonce)


def deserialize_signup_info(serialized_signup_info):
    return _PbftEnclaveSimulator.deserialize_signup_info(
        serialized_signup_info=serialized_signup_info)


def unseal_signup_data(sealed_signup_data):
    return _PbftEnclaveSimulator.unseal_signup_data(sealed_signup_data)


def release_signup_data(sealed_signup_data):
    return _PbftEnclaveSimulator.release_signup_data(sealed_signup_data)
...
def create_wait_timer(sealed_signup_data,
                      ...
                      local_mean):
    return \
        _PbftEnclaveSimulator.create_wait_timer(
            sealed_signup_data=sealed_signup_data,
            validator_address=validator_address,
            previous_certificate_id=previous_certificate_id,
            local_mean=local_mean)
...
def deserialize_wait_timer(serialized_timer, signature):
    return \
        _PbftEnclaveSimulator.deserialize_wait_timer(
            serialized_timer=serialized_timer,
            signature=signature)
...
def create_wait_certificate(sealed_signup_data, wait_timer, block_hash):
    return \
        _PbftEnclaveSimulator.create_wait_certificate(
            sealed_signup_data=sealed_signup_data,
            wait_timer=wait_timer,
            block_hash=block_hash)
...
def deserialize_wait_certificate(serialized_certificate, signature):
    return \
        _PbftEnclaveSimulator.deserialize_wait_certificate(
            serialized_certificate=serialized_certificate,
            signature=signature)


def verify_wait_certificate(certificate, pbft_public_key):
    _PbftEnclaveSimulator.verify_wait_certificate(
