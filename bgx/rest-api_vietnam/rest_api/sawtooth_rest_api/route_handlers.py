...
with open('../startup_global_state.json') as file:
    ...
    node = file_data['node']
    WALLETS[node['public_key']] = node['wallet']
    del node['wallet']
    NODE_CREDENTIALS = node
    for user in users:
        WALLETS[user['public_key']] = user['wallet']
...
def append_transaction(address_from, address_to, tx_payload, currency, extra):
    ...
    if currency not in WALLETS[NODE_CREDENTIALS['public_key']]:
        WALLETS[NODE_CREDENTIALS['public_key']][currency] = 0

    WALLETS[address_from][currency] -= tx_payload
    WALLETS[address_to][currency] += (tx_payload * (1 - TRANSACTION_FEE))
    WALLETS[NODE_CREDENTIALS['public_key']][currency] += tx_payload * TRANSACTION_FEE
    TRANSACTIONS.append({
...
class RouteHandler:
    ...
    async def post_wallet(self, request):
        ...
        user_address = rest_api_utils.pub_key_to_base64url(public_key)
        if public_key in WALLETS:
            return self._wrap_response(
                request,
                metadata={
                    public_key: {
    ...
    async def post_transaction_convert(self, request):
        ...
        address_from = rest_api_utils.pub_key_from_base64url(address_from)
        address_to = rest_api_utils.pub_key_from_base64url(address_to)
        ...
        result = rest_api_utils.verify_signature(address_from, signed_payload, payload)
        if result != 1:
    ...
    async def post_add_funds(self, request):
        ...
        # Verification of signed hashes
        result = rest_api_utils.verify_signature(address_to, signed_payload, payload)
        if result != 1:
            raise errors.InvalidSignature()
    ...
    async def get_wallet(self, request):
        address = request.match_info.get('address', '')
        address = rest_api_utils.pub_key_from_base64url(address)
