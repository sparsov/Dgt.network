...
def pub_key_to_base64url(pkcs1_DER_base64_pub_key):
    """
    :param pkcs1_DER_base64_pub_key: secp256k1 elliptic curve public key in Pkcs1 DER format in base64 encode
    :return: secp256k1 elliptic curve public key in Pkcs1 DER format in base64url encode
    """
    return pkcs1_DER_base64_pub_key.replace('/', '_').replace('+', '-')


def pub_key_from_base64url(pkcs1_DER_base64_pub_key):
    """
    :param pkcs1_DER_base64_pub_key: secp256k1 elliptic curve public key in Pkcs1 DER format in base64url encode
    :return: secp256k1 elliptic curve public key in Pkcs1 DER format in base64 encode
    """
    return pkcs1_DER_base64_pub_key.replace('_', '/').replace('-', '+')
...
def verify_signature(pub_key, signature, dict, pub_key_encode='base64'):
    """
    :param pub_key: secp256k1 elliptic curve public key in Pkcs1 DER format in <param>pub_key_encode</param> encode
    :param signature: secp256k1 signed sha256 hashed and base64 encoded json serialization of dict
    :param dict: dict which is signed
    :param pub_key_encode: encode of public key
    ...
    return chilkat_ecdsa.VerifyHashENC(sha256_hash, signature, pub_key_encode, chilkat_pub_key)
...
def generate_startup_global_state():
    ...
    node_state = {
        ...
        'address': pub_key_to_base64url(generated_keys[1]),
        'wallet': {
            'dec': 100,
            'bgt': 100000
        }
...
with open('../../startup_global_state.json') as file:
    ...
    node = file_data['node']
    print(pub_key_to_base64url(node['public_key']))
    for user in users:
        print(pub_key_to_base64url(user['public_key']))
