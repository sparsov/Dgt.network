...
ORIENT_URI = "uri://admin:foo@orientdb-test:2424/blocks"
_USE_ORIENT_ = True
...
class BlockStoreTest(unittest.TestCase):
    ...
    def test_my_chain_head(self):
        ...
        LOGGER.debug("TEST_MY_CHAIN_HEAD ..........")
        with block_store._block_store.cursor(index='block_num') as curs:
            curs.last()
            value = curs.value()
        LOGGER.debug("TEST_MY_CHAIN_HEAD val=%s",value)

    def test_chain_head(self):
        """ Test that the chain head can be retrieved from the
        BlockStore.
        """
        LOGGER.debug("TEST_CHAIN_HEAD ..........")
        block = self.create_block()
        block_store = self.create_block_store(
            {
                block.header_signature: block
        ...
        LOGGER.debug('TEST_CHAIN_HEAD chain_head=%s',chain_head)
        self.assert_blocks_equal(chain_head, block)
        LOGGER.debug('TEST_CHAIN_HEAD DONE\n')
    def test_get(self):
    ...
    def test_get_batch(self):
        ...
        LOGGER.debug('TEST_GET_BATCH batch_id=%s',batch_id)
        stored = block_store.get_batch(batch_id)
        LOGGER.debug('TEST_GET_BATCH batch_id stored=%s',stored)
        self.asset_protobufs_equal(stored, batch)
        LOGGER.debug('TEST_GET_BATCH protobufs_equal ok')
        with self.assertRaises(ValueError):
            stored = block_store.get_batch("bad")
        LOGGER.debug('TEST_GET_BATCH DONE')
    ...
    def create_block_store(data=None):
        if _USE_ORIENT_:
            ...
            if data is not None:
                print('CREATE_BLOCK_STORE ADD DATA',data.items())
                block_db.update(data.items(), [])
        else:
            block_db = DictDatabase(data,indexes=BlockStore.create_index_configuration())
        return BlockStore(block_db)
...
class BlockStorePredecessorIteratorTest(unittest.TestCase):
    ...
    def test_iterate_chain(self):
        ...
        if _USE_ORIENT_:
            ...
            block_db = DictDatabase(indexes=BlockStore.create_index_configuration())
    ...
    def test_iterate_chain_from_starting_block(self):
        ...
        if _USE_ORIENT_:
            ...
            block_db = DictDatabase(indexes=BlockStore.create_index_configuration())
        block_store = BlockStore(block_db)
        LOGGER.debug("TEST_ITERATE_CHAIN_FROM_STARTING_BLOCK ")
        chain = self._create_chain(5)
        print('TEST_ITERATE_CHAIN_FROM_STARTING_BLOCK chain\n\n',chain)
        block_store.update_chain(chain)
        print('TEST_ITERATE_CHAIN_FROM_STARTING_BLOCK update_chain\n\n')
        block = block_store['abcd2']
        print('TEST_ITERATE_CHAIN_FROM_STARTING_BLOCK block\n\n',block)
        ids = [b.identifier for b in block_store.get_predecessor_iter(block)]
        print('TEST_ITERATE_CHAIN_FROM_STARTING_BLOCK ids\n\n',ids,'\n\n')
        self.assertEqual(
            ['abcd2', 'abcd1', 'abcd0'],
            ids)
        LOGGER.debug("TEST_ITERATE_CHAIN_FROM_STARTING_BLOCK DONE")
