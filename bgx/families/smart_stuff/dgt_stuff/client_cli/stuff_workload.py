...
from dgt_stuff.client_cli.create_batch import create_batch
from dgt_stuff.client_cli.exceptions import StuffCliException
from sawtooth_sdk.protobuf import batch_pb2

LOGGER = logging.getLogger(__name__)

StuffState = namedtuple('StuffState', ['name', 'url', 'value'])
...
class StuffWorkload(Workload):
    ...
    def __init__(self, delegate, args):
        super(StuffWorkload, self).__init__(delegate, args)
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        StuffState(name=name, url=url, value=0)
...
def do_workload(args):
    """
    Create WorkloadGenerator and StuffWorkload. Set Bgt workload in
    generator and run.
    """
    try:
        args.auth_info = _get_auth_info(args.auth_user, args.auth_password)
        generator = WorkloadGenerator(args)
        workload = StuffWorkload(generator, args)
